...
isLeftWhiteHLine :: Image PixelRGB8 -> Int -> Int -> Bool
...
main = do
    ...
    let Right (ImageRGB8 img) = decodePng c
        ...
        myPred = isLeftWhiteHLine img
        Just hMid = listToMaybe . filter (myPred 43) $ concat
            [[h, imgH - h] | h <- [imgHHalf, imgHHalf - 1  .. 1]]
        h1 = last $ takeWhile (myPred 43) [hMid .. imgH - 1]
        h2 = last $ takeWhile (myPred 43) [hMid, hMid - 1 .. 0]
    {-
